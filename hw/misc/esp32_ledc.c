...
static uint64_t esp32_ledc_read(void *opaque, hwaddr addr, unsigned int size)
{
	...
	r = s->duty_reg[(addr - A_LEDC_HSCH0_DUTY_REG) / 0x14];
	break;
...
static void get_duty_time(Esp32LEDCState *s, int index, uint32_t *on_time, uint32_t *off_time) {
		...
		if (index < 8) {
            ...
            duty_res = s->duty_res[(s->channel_conf0_reg[index] & ((1 << 2) - 1)) + 4];
        }
                            ...
                            uint64_t value, unsigned int size)
{
    ...
    case A_LEDC_LSCH6_CONF1_REG:
    case A_LEDC_LSCH7_CONF1_REG:
        s->channel_conf1_reg[(addr - A_LEDC_HSCH0_CONF1_REG) / 0x14] = value;
        break;
    case A_LEDC_HSCH0_DUTY_REG:
    case A_LEDC_HSCH1_DUTY_REG:
    case A_LEDC_HSCH2_DUTY_REG:
    ...
    case A_LEDC_LSCH7_DUTY_REG:
        ...
        led_set_intensity(&s->led[index], esp32_ledc_get_percent(s, value, addr));
	    s->op_val[index]=1;
		qemu_set_irq(s->func_irq,(71+index)*2+1+((on_time/1000)<<10));
        s->cycle[index]=0;
        timer_mod_anticipate_ns(&s->led_timer[index],qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL)+on_time);
        break;
    case A_LEDC_INT_ENA:
        s->int_en=value;
...
static void ledc_timer_cb(void *v) {
    ...
    if(s->op_val[index]==0) {
	    ...
	    timer_mod_anticipate_ns(&s->led_timer[index],qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL)+off_time);
        ...
        if(duty_num!=0) { // fade control
            ...
            if(s->cycle[index]>=duty_cycle) {
                ...
                s->channel_conf1_reg[index]=(s->channel_conf1_reg[index]&0xc00fffff) | duty_num<<20;
            }
        }
    }
}
...
static void esp32_ledc_init(Object *obj)
{
    ...
    current_state=s;
    for(int i = 0; i < ESP32_LEDC_CHANNEL_CNT; i++) {
        timer_init_ns(&s->led_timer[i],QEMU_CLOCK_VIRTUAL, ledc_timer_cb, (void *)(long long)i);
    }
